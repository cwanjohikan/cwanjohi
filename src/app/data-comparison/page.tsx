    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const jsonData: (string | number | boolean | Date | null | undefined)[][] = XLSX.utils.sheet_to_json(worksheet, { header: 1 })
          
          if (jsonData.length < 2) {
            throw new Error('Excel file must have at least a header row and one data row')
          }
          
          const headers = jsonData[0].map((h: unknown) => (h != null ? String(h).toLowerCase() : ''))
